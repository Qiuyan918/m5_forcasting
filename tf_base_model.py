...
class TFBaseModel(object):
    ...
    def predict(self, mode, chunk_size=256, suffix=None):
        if hasattr(self, 'prediction_tensors'):
            ...
            if mode == 'val_test':
                ...
                test_generator = self.reader.test_batch_generator(chunk_size, mode)
                
            elif mode == 'test':
                if not os.path.isdir(self.submission_dir):
                    os.makedirs(self.submission_dir)
                prediction_dir = self.submission_dir
                test_generator = self.reader.test_batch_generator(chunk_size, mode)                
            ...
            for tensor_name, tensor in prediction_dict.items():
                ...
                save_file = os.path.join(prediction_dir, '{}_{}.npy'.format(tensor_name, suffix))
    ...
    def restore(self, step=None, averaged=False):
        ...
        if not step:
            ...
            model_path = os.path.join(
                checkpoint_dir, 'model{}-{}'.format('' if averaged else '', step)
            )
